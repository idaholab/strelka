...
class ScanEmail(Scanner):
        ...
        try:
                ...
                for body in parsed_eml["body"]:
                    if body_enabled:
                        if "content_type" in body:
                            ...
                            elif body["content_type"] == "text/html" and "body" not in self.event:
                                text = BeautifulSoup(body["content"], "html.parser").get_text(separator=" ")
            ...
            if rewrite_mode != "none" and rewrite_rules:
                ...
                if rewrite_mode == "replace":
                    ...
                    links_unwrapped = [u for orig, u in zip(deduped_links, unwrapped) if u != orig]
                    ...
                    self.event["links_unwrapped"] = [u for orig, u in zip(deduped_links, unwrapped) if u != orig]
            ...
            if date_header:
                self.event["date_utc"] = date_header.astimezone(pytz.utc).isoformat()[:-6] + ".000Z"
            header = parsed_eml.get("header", {}).get("header", {})
            message_id = header.get("message-id", [])[0] if header else None
            self.event["message_id"] = str(message_id.lstrip("<").rstrip(">")) if message_id else ""
            self.event["received_domain"] = parsed_eml["header"].get("received_domain", [])
            ...
            if attachments:
                for attachment in attachments:
                    ...
                    try:
                        ...
                        self.flags.append(f"{self.__class__.__name__}: email_extract_attachment_error: {str(e)[:50]}")
    ...
    def _extract_curated_headers(self, parsed_header: dict, raw: dict, display_name_mode: str = "none") -> None:
        ...
        def emit_addr_field(field: str, pairs: list) -> None:
            ...
            if display_name_mode == "copy":
                ...
                self.event[f"{field}_mailbox"] = [f"{name} <{addr}>" if name else addr for name, addr in pairs]
            else:
                self.event[field] = specs
        ...
        self.event["in_reply_to"] = self._header_value(raw, "in-reply-to").lstrip("<").rstrip(">")
        self.event["thread_topic"] = self._decode_mime_words(self._header_value(raw, "thread-topic"))
        self.event["x_originating_ip"] = self._header_value(raw, "x-originating-ip").lstrip("[").rstrip("]")
        ...
        self.event["references"] = (
            [ref.lstrip("<").rstrip(">") for ref in references_raw.split()] if references_raw else []
        )
    ...
    def _build_raw_header_map(self, raw: dict, options: dict) -> tuple:
        ...
        for raw_name, values in raw.items():
            ...
            entry_size = len(name.encode("utf-8")) + sum(len(v.encode("utf-8")) for v in out_values)
