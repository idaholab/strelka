...
from .common.pygments import Highlight, PygmentsScanner, IndicatorToken


class ScanVb(PygmentsScanner):
    """
    Collects metadata from Visual Basic (VB) script files.
    """

    TOKEN_MAPPING: ClassVar = {
        "comments": {"Token.Comment"},
        "functions": {"Token.Name.Function"},
        "names": {"Token.Name"},
        "operators": {"Token.Operator"},
        "strings": {"Token.Literal.String"},
        "urls": {IndicatorToken.url},
    }

    def get_lexer(self) -> lexers.Lexer:
        return lexers.get_lexer_by_name("vbnet")

    def process_entry(self, entry: Highlight) -> Iterator[Highlight]:
        match entry.token:
            case "Token.Comment" | "Token.Literal.String":
                yield from self.extract_urls(entry.value)
        yield entry
