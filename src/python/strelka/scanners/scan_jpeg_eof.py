        ...
        try:
            ...
            try:
                if not data[offset:].startswith(b"\xff\xd8"):
                    self.add_flag("corrupt_jpeg_data_no_soi")
                    return
            except IndexError:
                self.add_flag("jpeg_bad_data_offset")
                return
            ...
            while True:
                ...
                if marker[0] != 0xFF:
                    self.add_flag("corrupt_jpeg_data_misaligned_marker")
                    break
            ...
            self.event["trailer_start"] = offset
            self.emit_file(
                data[offset:],
                name=":jpeg-trailer",
            )
            ...
            self.add_flag("jpeg_general_parsing_error")
